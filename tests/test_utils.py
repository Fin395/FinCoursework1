...
def test_filter_transactions_by_period(sample_transactions: list[dict]) -> None:
...
def test_get_top_transactions(sample_top_transactions: list[dict]) -> None:
    ...
    assert get_top_transactions(sample_top_transactions) == [
        {"amount": -45658.0, "category": "Красота", "date": "03.01.2018", "description": "OOO Balid"},
        {"amount": -3000.0, "category": "Переводы", "date": "01.01.2018", "description": "Линзомат ТЦ Юность"},
        {"amount": -745.0, "category": "Красота", "date": "04.01.2018", "description": "OOO Balid"},
        {"amount": -50.0, "category": "Переводы", "date": "01.01.2018", "description": "Линзомат ТЦ Юность"},
        {"amount": -21.0, "category": "Красота", "date": "01.03.2018", "description": "OOO Balid"},
    ]
...
@pytest.mark.parametrize(
    "sample_path_to_json_file, expected",
    [(JSON_DIR, ["AAPL", "AMZN", "GOOGL", "MSFT", "TSLA"]), ("user_settings", [])],
)
...
def test_get_currency_rate(mock_request: Any) -> None:
    ...
    mock_request.return_value.json.return_value = [
        {"base": "USD", "rates": {"RUB": 74.098804}},
        {"base": "EUR", "date": "2021-12-20", "rates": {"RUB": 83.567092}},
    ]
...
def test_get_stock_price(mock_request: Any) -> None:
    ...
    mock_request.return_value.json.return_value = [
        {"data": [{"close": 254.49, "symbol": "AAPL"}]},
        {"data": [{"close": 224.92, "symbol": "AMZN"}]},
        {"data": [{"close": 191.41, "symbol": "GOOGL"}]},
        {"data": [{"close": 436.6, "symbol": "MSFT"}]},
        {"data": [{"close": 421.06, "symbol": "TSLA"}]},
    ]
    result = get_stock_price(["AAPL", "AMZN", "GOOGL", "MSFT", "TSLA"], "2024-12-20 23:59:59")
    assert result == [
        {"stock": "AAPL", "price": 254.49},
        {"stock": "AMZN", "price": 224.92},
        {"stock": "GOOGL", "price": 191.41},
        {"stock": "MSFT", "price": 436.6},
        {"stock": "TSLA", "price": 421.06},
    ]
